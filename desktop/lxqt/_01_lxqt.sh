...
configure_lxqt_session_and_appearance(){
    ...
    term_color_red
    echo ">>> Please update the Global Screen Scaling value"
    term_color_white
    lxqt-config-session
    ...
    term_color_red
    echo ">>> Please go to the Mouse and Touchpad and enable the options"
    term_color_white
    lxqt-config-input
...
Elan%2520Touchpad\tapToDragEnabled=1
Elan%2520Touchpad\tappingEnabled=1

    # Disable Idleness Watcher
    # ~/.config/lxqt/lxqt-powermanagement.conf => enableIdlenessWatcher=false
    term_color_red
    echo ">>> Please go to the Idle and disable the watcher option"
    term_color_white
